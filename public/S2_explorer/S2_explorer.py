def udf(bbox, provider='AWS',channels = ['B11','veg','snow'],time_of_interest = "2023-05-01/2023-09-13", cloud_cover_perc = 10, brightness = 1.0, perc_arr = [50]*3, palette = 'terrain_r'):
    import os
    import rasterio
    ...
    import odc.stac 
    from gabeutils import normalize, channel_band_dict, reverse_channel_band_dict, norm_dict, arr_to_cmap, color_dict_256
    
    index_name_dict = {'burn':['B08','B12'],
                       'glacier':['B03','B04'],
                       'moisture':['B08','B11'],
                       'snow':['B03','B11'],
                       'veg':['B08','B04'],
    ...
    if len(items) ==0:
        print('no satellite imagery available for the current viewport and time period. Please explore other regions or timeframes.')
