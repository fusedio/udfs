@fused.udf
def udf(bounds: fused.types.TileGDF=None, resolution: int = 11, min_count: int = 10):
    import duckdb
    import shapely
    ...
    tile_bbox_geom = bounds if bounds is not None else default_bbox

    bounds = bounds.bounds.values[0] if bounds is not None else default_bbox.bounds
    print(bounds)
