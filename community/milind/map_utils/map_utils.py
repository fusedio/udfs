...
def deckgl_raster_tiles(
    ...
    """
    Render raster tiles (PNG/JPG) using Mapbox GL's native raster source.
    
    Args:
        tile_url: Tile URL template with {z}/{x}/{y} placeholders
                  e.g., "https://udf.ai/UDF_CDL_mask/run/tiles/{z}/{x}/{y}?dtype_out_raster=png"
        config: Optional dict with initialViewState and rasterLayer config
        mapbox_token: Mapbox access token
    ...
    Example:
        html = map_utils.deckgl_raster_tiles(
            tile_url="https://unstable.udf.ai/UDF_CDL_mask/run/tiles/{z}/{x}/{y}?dtype_out_raster=png",
            config={
                "initialViewState": {"longitude": -100, "latitude": 40, "zoom": 5},
    ...
    if config is None:
        ...
        if "initialViewState" in config:
            merged["initialViewState"].update(config["initialViewState"])
        if "rasterLayer" in config:
            merged["rasterLayer"].update(config["rasterLayer"])
        config = merged
    
    ivs = config.get("initialViewState", {})
    center_lng = ivs.get("longitude", -98.5)
    center_lat = ivs.get("latitude", 39.8)
    zoom = ivs.get("zoom", 4)
    pitch = ivs.get("pitch", 0)
    bearing = ivs.get("bearing", 0)
    
    raster_cfg = config.get("rasterLayer", {})
    opacity = raster_cfg.get("opacity", 1.0)
    min_zoom = raster_cfg.get("minZoom", 0)
    max_zoom = raster_cfg.get("maxZoom", 22)
    tile_size = raster_cfg.get("tileSize", 256)
...
<body>
<div id="map"></div>
<div id="tile-loader"><div class="spinner"></div><span id="loader-text">Loading tiles...</span></div>
<div class="controls">
  <label>Opacity: <span id="opacity-val">{{ opacity }}</span></label>
  <input type="range" id="opacity-slider" min="0" max="1" step="0.05" value="{{ opacity }}">
</div>
...
function updateLoader(delta) {
  tilesLoading = Math.max(0, tilesLoading + delta);
  const loader = document.getElementById('tile-loader');
  ...
  if (tilesLoading > 0) {
    ...
    loader.classList.add('visible');
    text.textContent = tilesLoading === 1 ? 'Loading tile...' : `Loading ${tilesLoading} tiles...`;
  } else {
    loaderTimeout = setTimeout(() => loader.classList.remove('visible'), 300);
  }
}
...
map.on('load', () => {
  ...
  // Loading events
  map.on('sourcedataloading', (e) => {
    if (e.sourceId === 'raster-tiles' && e.tile) updateLoader(1);
  });
  map.on('sourcedata', (e) => {
    if (e.sourceId === 'raster-tiles' && e.tile) updateLoader(-1);
  });
  ...
  document.getElementById('opacity-slider').addEventListener('input', (e) => {
    ...
    map.setPaintProperty('raster-layer', 'raster-opacity', val);
