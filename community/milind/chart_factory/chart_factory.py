...
def udf(
    ...
    # Create the chart using the factory functions
    if library == "echarts":
...
def create_chart_html(chart_type, library, data, params):
    """Create chart HTML based on chart type and library"""
    ...
    if library == "echarts":
        if chart_type == "line":
            ...
            <head>
                ...
                <style>
                    ...
                    #chart {{ width: 100%; height: {params['height']}px; }}
                </style>
            </head>
            <body>
                <h2>{params['title']}</h2>
                ...
                <script>
                    ...
                    var option = {{
                        ...
                        xAxis: {{ type: 'category', data: {data[params['x_column']].dt.strftime('%Y-%m-%d').tolist()} }},
                        yAxis: {{ type: 'value' }},
                        series: [{{
                            data: {data[params['y_column']].tolist()},
                            type: 'line',
                            smooth: true
                        }}]
                    }};
            ...
            category_data = data.groupby(params['category_column'])[params['value_column']].sum()
            return f"""
            <!DOCTYPE html>
            <html>
            <head>
            ...
            <body>
                ...
                <script>
                    var chart = echarts.init(document.getElementById('chart'));
                    var option = {{
                        title: {{ text: '{params['title']}', left: 'center' }},
            ...
            <body>
                ...
                <script>
                    var chart = echarts.init(document.getElementById('chart'));
                    var data = {list(zip(data[params['x_column']], data[params['y_column']]))};
                    var option = {{
                        title: {{ text: '{params['title']}' }},
                        xAxis: {{ type: 'value', name: '{params['x_column']}' }},
                        yAxis: {{ type: 'value', name: '{params['y_column']}' }},
                        series: [{{
                            type: 'scatter',
                            data: data
                        }}]
        ...
        if chart_type == "line":
            ...
            chart = alt.Chart(data).mark_arc().encode(
                theta=params['value_column'],
                color=params['category_column']
            ).properties(title=params['title'])
        elif chart_type == "scatter":
