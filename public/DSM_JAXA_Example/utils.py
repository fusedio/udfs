...
def dsm_to_tile(bbox, z_levels=[4, 6, 9, 11], verbose=True):
        if bbox.z[0]>=z_levels[2]:
            tiff_list = []
            if bbox.z[0]>=z_levels[3]: overview_level=0
            else: overview_level=1
            if verbose: print(f'{overview_level=}')
            bounds_list = bbox_to_navigation(bbox, interval=1)
            
            for b in bounds_list:
                zoom_level=3             
                tiff_list.append(f'https://s3.ap-northeast-1.wasabisys.com/je-pds/cog/v1/JAXA.EORC_ALOS.PRISM_AW3D30.v3.2_global/2021-02/{zoom_level}/{b[0]}-{b[1]}/{b[0]}-{b[2]}-{b[1]}-{b[3]}-DSM.tiff')
        elif bbox.z[0]>=z_levels[0]:
            tiff_list = []
            if bbox.z[0]>=z_levels[1]: overview_level=0
            else: overview_level=1
            if verbose: print(f'{overview_level=}')
            ...
            return bbox
        if verbose: print(tiff_list)
        arr = mosaic_tiff(bbox,tiff_list, overview_level=overview_level)
        return arr 
    ...
    if interval==1:
        for lat in  range(int(np.floor(bounds[1])),int(np.ceil(bounds[3]))):
            for lng in  range(int(np.floor(bounds[0])),int(np.ceil(bounds[2]))):
                a.append([convert_lng(lng),convert_lng(lng+1),convert_lat(lat),convert_lat(lat+1)])
    elif interval==10:
        for lat in  range(int(np.floor(bounds[1]/10)*10),int(np.ceil(bounds[3]/10)*10),10):
            for lng in  range(int(np.floor(bounds[0]/10)*10),int(np.ceil(bounds[2]/10)*10),10):
                a.append([convert_lng(lng),convert_lng(lng+10),convert_lat(lat),convert_lat(lat+10)])
    return a
