...
def read_tiff(bbox, input_tiff_path, crs, buffer_degree, output_shape, resample_order=0):
    ...
    with rasterio.Env(AWSSession(requester_pays=True)):
        with rasterio.open(input_tiff_path) as src:
            ...
            rgb=np.array([zoom(arr, zoom_factors, order=resample_order) for arr in data])
        return rgb
