...
def udf(engine="realtime", year: str = "2016", month: str = "07", period: str = "a"):
    ...
    def load(crop_type="corn"):
        gdf_counties = gpd.read_parquet(
            "s3://fused-asset/data/tiger/county/tl_rd22_us_county small.parquet"
        )
        target_counties = gdf_counties[
            gdf_counties["GEOID"].str.startswith(("31", "19", "17", "18"))
        ].GEOID.values

        # Create array of all parameter combinations
        all_params = [
            {"geoid": geoid, "year": year, "crop_type": crop_type}
            for geoid in target_counties
        ]
...
def udf(
    ...
    from utils2 import clip_arr, get_da, get_da_bounds, get_masked_array

    xy_cols = ["lon", "lat"]

    # Get the geometry
    gdf = gpd.read_parquet(
        "s3://fused-asset/data/tiger/county/tl_rd22_us_county 25pct.parquet"
    )
    ...
    arr_aoi = clip_arr(
        da.values,
        bounds_aoi=geom_bbox.total_bounds,
    ...
    # Load crop UDF
    udf = fused.load(
        "https://github.com/fusedio/udfs/tree/ba8aeee/public/CDLs_Tile_Example/"
    )
    arr_crop = fused.run(
        udf,
        engine="local",
    ...
    sif_resized = resize(
        img,
        (arr_crop.shape[0], arr_crop.shape[1]),
        anti_aliasing=True,
        preserve_range=True,
    ).astype("uint8")
        ...
        [
            {
                ...
                f"{crop_type}_sif_mean": sif_resized_county_corn.sum()
                / (sif_resized_county_corn > 0).sum()
                / 255,
                "sif_ratio": (
                    sif_resized_county_corn.sum() / (sif_resized_county_corn > 0).sum()
                )
                / sif_resized_county.mean(),
                "GEOID": geoid,
