@fused.udf
def udf(engine='realtime', year: str = "2020", month: str = "05", period: str ="a", geoids: tuple =('08','16', '17', '18','19','20','21','26', '27','29', '31','38', '39','46','47','55')):
    import geopandas as gpd
    import pandas as pd
    # geoids = ('31', '19', '17', '18')
    if isinstance(geoids, str):
        import json
        geoids = json.loads(geoids)
        geoids = tuple(f"{num:02}" for num in geoids)


    # Get the geometry
    @fused.cache
    def load(crop_type='corn', geoids=geoids,year=year, month=month, period=period):
        
        gdf_counties = gpd.read_parquet('s3://fused-asset/data/tiger/county/tl_rd22_us_county small.parquet')
        target_counties = gdf_counties[gdf_counties['GEOID'].str.startswith(geoids)].GEOID.values
...
@fused.udf
def udf_nail(crop_type ="corn", geoid: str = '19119', year: str = "2020", month: str = "05", period: str ="a", output_suffix='out1'):
