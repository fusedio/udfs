...
def udf():
    ...
    # Multiple URLs per year as fallbacks
    survey_urls = {
        2020: [
            "https://info.stackoverflowsolutions.com/rs/719-EMH-566/images/stack-overflow-developer-survey-2020.zip",
            "https://survey.stackoverflow.co/datasets/stack-overflow-developer-survey-2020.zip",
        ],
        2021: [
    ...
    def download_survey(year):
        """Try each URL for a year until one works"""
        urls = survey_urls[year]
        for url in urls:
            try:
                response = requests.get(url, headers=headers, allow_redirects=True, timeout=120)
                response.raise_for_status()
                if b'PK' not in response.content[:4]:
                    print(f"Not a zip from {url}")
                    continue
                with zipfile.ZipFile(io.BytesIO(response.content)) as zip_file:
                    for filename in ['survey_results_public.csv', 'survey_results.csv']:
                        try:
    ...
    def get_python_stats(year):
        try:
            df = download_survey(year)
            if df is None:
                return None
            total_responses = len(df)
            language_col = None
            for col in ['LanguageHaveWorkedWith', 'LanguageWorkedWith', 'HaveWorkedLanguage']:
            ...
            return {
                ...
                'python_users': int(python_users),
                'python_percentage': round(python_percentage, 2)
            }
        except Exception as e:
    ...
    print(f"Successfully loaded {len(stats)} years: {[s['year'] for s in stats]}")
    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .header h1 { font-size: 2.5rem; font-weight: 300; margin-bottom: 10px; letter-spacing: -0.5px; }
            ...
            .chart-card {
                ...
                transition: transform 0.3s ease; max-width: 800px; margin: 0 auto;
            }
            .chart-card:hover { transform: translateY(-5px); }
            .chart-title { font-size: 1.5rem; font-weight: 300; color: #333; margin-bottom: 30px; text-align: center; }
            .chart-container { position: relative; height: 500px; }
            @media (max-width: 767px) {
                body { padding: 20px 10px; }
                .chart-card { padding: 30px 20px; }
        ...
        <script>
            ...
            new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [
                        {
                            label: 'Python Usage %', data: data.map(d => d.python_percentage),
                            borderColor: '#3776ab', backgroundColor: 'rgba(55,118,171,0.1)',
                            borderWidth: 3, fill: false, tension: 0.4,
                            pointBackgroundColor: '#3776ab', pointBorderColor: '#fff',
                            pointBorderWidth: 2, pointRadius: 5, pointHoverRadius: 7, yAxisID: 'y'
                        },
                        {
                            label: 'Total Python Users', data: data.map(d => d.python_users),
                            borderColor: '#ff6b6b', backgroundColor: 'rgba(255,107,107,0.1)',
                            borderWidth: 3, fill: false, tension: 0.4,
                            pointBackgroundColor: '#ff6b6b', pointBorderColor: '#fff',
                            pointBorderWidth: 2, pointRadius: 5, pointHoverRadius: 7, yAxisID: 'y1'
                        }
                    ]
                },
                options: {
                    ...
                    plugins: {
                        ...
                        tooltip: { callbacks: { label: function(context) {
                            ...
                            return context.datasetIndex === 0 ? label + context.parsed.y.toFixed(1) + '%' : label + formatNumber(context.parsed.y);
                        }}}
                    },
                    scales: {
                        x: { display: true, title: { display: true, text: 'Year', font: { size: 14 } }, grid: { display: false } },
                        y: { type: 'linear', display: true, position: 'left',
                            title: { display: true, text: 'Usage Percentage (%)', font: { size: 14 } },
                            beginAtZero: true, grid: { color: 'rgba(0,0,0,0.05)' },
                            ticks: { callback: function(value) { return value.toFixed(1) + '%'; }, font: { size: 12 } }
                        },
                        y1: { type: 'linear', display: true, position: 'right',
                            title: { display: true, text: 'Total Users', font: { size: 14 } },
                            beginAtZero: true, grid: { drawOnChartArea: false },
                            ticks: { callback: function(value) { return formatNumber(value); }, font: { size: 12 } }
                        }
                    }
                }
            });
