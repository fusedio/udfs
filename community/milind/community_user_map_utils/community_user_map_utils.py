...
<head>
  ...
  <style>
    ...
    #location-search input::placeholder { color: var(--ui-muted-2); }
    #location-search input:focus { border-color: var(--ui-elements); }
    ...
    #location-search .search-results.visible { display: block; }
    ...
    #location-search .search-item:last-child { border-bottom: none; }
    #location-search .search-item:hover { background: var(--ui-surface); }
    #location-search .search-item .place-name { font-weight: 500; color: var(--ui-text); }
    #location-search .search-item .place-context { font-size: 10px; color: var(--ui-muted); margin-top: 2px; }
    
    /* Push layer panel down to make room for search */
      ...
      function renderResults(features) {
        ...
        searchResults.querySelectorAll('.search-item').forEach(item => {
          item.addEventListener('click', () => {
            ...
            searchInput.value = item.querySelector('.place-name').textContent;
            searchResults.classList.remove('visible');
          });
        });
      }
