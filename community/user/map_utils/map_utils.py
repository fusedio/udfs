...
def deckgl_layers(
    ...
    for i, layer_def in enumerate(layers):
        ...
        if layer_type == "hex":
            ...
            tile_url = layer_def.get("tile_url")
            source_layer = layer_def.get("source_layer", "udf")  # For Fused MVT tiles, source-layer is 'udf'
            tile_layer_config = merged_config.get("tileLayer", {})
            
            is_tile_layer = tile_url is not None
            is_mvt = is_tile_layer and ('mvt' in tile_url.lower() or tile_url.endswith('.pbf'))
            if is_tile_layer and not is_mvt:
                has_tile_layers = True  # GeoJSON tiles use Deck.gl (MVT uses Mapbox GL native)
            
            # Convert GeoDataFrame to GeoJSON (for static layers)
            geojson_obj = {"type": "FeatureCollection", "features": []}
            if not is_tile_layer and df is not None and hasattr(df, "to_json"):
...
<body>
  ...
  <script>
    ...
    function buildVectorTileLayer(layerDef, beforeId = null) {
      const tileUrl = layerDef.tileUrl;
      const tileCfg = layerDef.tileLayerConfig || {};
      const vectorCfg = layerDef.vectorLayer || {};
      const visible = layerVisibility[layerDef.id];
      const layerOpacity = (typeof vectorCfg.opacity === 'number') ? vectorCfg.opacity : 0.8;
      const isFilled = layerDef.isFilled !== false;
      const isStroked = layerDef.isStroked !== false;
      const lineWidth = layerDef.lineWidth || 1;
      const pointRadius = layerDef.pointRadius || 6;
      
      // Parse color configs
      const fillColorCfg = layerDef.fillColorConfig || {};
      const lineColorCfg = layerDef.lineColorConfig || {};
      
      // Build color accessor functions
      function buildColorAccessor(cfg, fallbackRgba, defaultColor) {
        if (cfg?.['@@function'] === 'colorContinuous' && cfg.domain?.length >= 2) {
          const [d0, d1] = cfg.domain;
          const rev = d0 > d1;
          const dom = rev ? [d1, d0] : [d0, d1];
          const paletteName = cfg.colors || 'TealGrn';
          const steps = cfg.steps || 7;
          let cols = getPaletteColors(paletteName, steps);
          if (!cols) cols = ['#e0f3db','#a8ddb5','#7bccc4','#4eb3d3','#2b8cbe','#0868ac'];
          if (rev) cols = [...cols].reverse();
          
          return (d) => {
            const val = d?.properties?.[cfg.attr] ?? d?.[cfg.attr];
            if (val == null || !isFinite(val)) return cfg.nullColor || [128, 128, 128, 150];
            const t = Math.max(0, Math.min(1, (val - dom[0]) / (dom[1] - dom[0])));
            const idx = Math.min(Math.floor(t * cols.length), cols.length - 1);
            const hex = cols[idx];
            // Parse hex to RGB
        ...
        if (cfg?.['@@function'] === 'colorCategories') {
          ...
          let cols = getPaletteColors(paletteName, 10);
          if (!cols) cols = ['#7F3C8D','#11A579','#3969AC','#F2B701','#E73F74','#80BA5A','#E68310','#008695','#CF1C90','#f97b72'];
          const catMap = new Map();
          
          return (d) => {
            const val = d?.properties?.[cfg.attr] ?? d?.[cfg.attr];
            if (val == null) return cfg.nullColor || [128, 128, 128, 150];
            if (!catMap.has(val)) catMap.set(val, catMap.size);
            const idx = catMap.get(val) % cols.length;
            const hex = cols[idx];
            const r = parseInt(hex.slice(1, 3), 16);
            const g = parseInt(hex.slice(3, 5), 16);
            const b = parseInt(hex.slice(5, 7), 16);
        ...
        if (fallbackRgba) {
          const match = fallbackRgba.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
          if (match) return [parseInt(match[1]), parseInt(match[2]), parseInt(match[3]), 200];
        }
        return defaultColor;
      }
      
      const getFillColor = isFilled 
        ? buildColorAccessor(fillColorCfg, layerDef.fillColorRgba, [0, 144, 255, 150])
        : [0, 0, 0, 0];
      const getLineColor = isStroked
        ? buildColorAccessor(lineColorCfg, layerDef.lineColorRgba, [255, 255, 255, 200])
        : [0, 0, 0, 0];

      const layerProps = {
        id: `${layerDef.id}-vector-tiles`,
        data: tileUrl,
        tileSize: tileCfg.tileSize ?? 256,
        minZoom: tileCfg.minZoom ?? 0,
        maxZoom: tileCfg.maxZoom ?? 14,
        pickable: true,
        visible: visible,
        maxRequests: 6,
        ...
        getTileData: async ({ index, signal }) => {
          ...
          const cacheKey = `vec|${tileUrl}|${z}|${x}|${y}`;
          ...
          try {
            ...
            const geojson = data.type === 'FeatureCollection' ? data : 
                           data.features ? { type: 'FeatureCollection', features: data.features } :
                           { type: 'FeatureCollection', features: Array.isArray(data) ? data : [] };
            TILE_CACHE.set(cacheKey, geojson);
            updateTileLoader(-1);
            return geojson;
          } catch (e) {
            updateTileLoader(-1);
            return TILE_CACHE.get(cacheKey) || { type: 'FeatureCollection', features: [] };
          }
        },
    ...
    function rebuildDeckOverlay() {
      // Get all tile layers (both hex and vector)
      const hexTileLayers = LAYERS_DATA
        .filter(l => l.layerType === 'hex' && l.isTileLayer && layerVisibility[l.id]);
      const vectorTileLayers = LAYERS_DATA
        .filter(l => l.layerType === 'vector' && l.isTileLayer && layerVisibility[l.id]);
      const allTileLayers = [...hexTileLayers, ...vectorTileLayers];
      ...
      // Build deck layers - reverse to keep UI order consistent
      const deckLayers = [];
      
      // Add hex tile layers
      hexTileLayers.slice().reverse().forEach(l => {
        deckLayers.push(buildTileLayer(l, bottomMostLayerId));
      });
      
      // Add vector tile layers
      vectorTileLayers.slice().reverse().forEach(l => {
        deckLayers.push(buildVectorTileLayer(l, bottomMostLayerId));
      });
    ...
    function addAllLayers() {
      ...
      renderOrder.forEach((l) => {
        // Skip Deck.gl tile layers, but handle MVT tiles here (Mapbox GL native)
        if (l.isTileLayer && !l.isMvt) return;
        
        const visible = layerVisibility[l.id];
        
        // Handle MVT (Mapbox Vector Tiles) with native Mapbox GL source
        if (l.isMvt && l.tileUrl) {
          map.addSource(l.id, {
            type: 'vector',
            tiles: [l.tileUrl],
            minzoom: l.tileLayerConfig?.minZoom ?? 0,
            maxzoom: l.tileLayerConfig?.maxZoom ?? 14
          });
          
          const sourceLayer = l.sourceLayer || 'default';
          const fillColorExpr = l.fillColorConfig?.['@@function'] ? buildColorExpr(l.fillColorConfig, []) : (l.fillColorRgba || 'rgba(0,144,255,0.6)');
          const lineColorExpr = l.lineColorConfig?.['@@function'] ? buildColorExpr(l.lineColorConfig, []) : (l.lineColorRgba || 'rgba(100,100,100,0.8)');
          ...
          // Add line layer for strokes and lines
    ...
    function tryInit() {
      ...
      map.on('mousemove', (e) => {
        ...
        {% if has_tile_layers %}
        // Check Deck.gl tile layers (both hex and vector)
        if (deckOverlay) {
          const info = deckOverlay.pickObject({ x: e.point.x, y: e.point.y, radius: 4 });
          if (info?.object) {
            // Extract layer ID from various tile layer ID formats
            let layerId = info.layer?.id || '';
            layerId = layerId.replace(/-tiles-[^-]+$/, '').replace(/-vector-tiles.*$/, '').replace(/-tiles$/, '').replace(/-geojson$/, '');
            const layerDef = LAYERS_DATA.find(l => l.id === layerId || layerId.startsWith(l.id));
        ...
        if (bestFeature.type === 'mapbox') {
          ...
          // Deck.gl tile layer (hex or vector)
          const obj = bestFeature.object;
          const p = obj?.properties || obj || {};
          const isHexLayer = layerDef.layerType === 'hex';
          const colorAttr = isHexLayer 
            ? (layerDef.hexLayer?.getFillColor?.attr || 'metric')
            : (layerDef.fillColorConfig?.attr || layerDef.lineColorConfig?.attr);
          
          // Show hex ID for hex layers
          const hexVal = p.hex || obj.hex;
          if (isHexLayer && hexVal) {
          ...
          } else if (colorAttr && (p[colorAttr] != null || obj[colorAttr] != null)) {
            const val = p[colorAttr] ?? obj[colorAttr];
            lines.push(`<span class="tt-row"><span class="tt-key">${colorAttr}</span><span class="tt-val">${typeof val === 'number' ? val.toFixed(2) : val}</span></span>`);
          } else if (!isHexLayer) {
            // For vector tiles without specific columns, show first few properties
            Object.keys(p).slice(0, 5).forEach(k => {
              if (k !== 'geometry' && p[k] != null) {
                lines.push(`<span class="tt-row"><span class="tt-key">${k}</span><span class="tt-val">${typeof p[k] === 'number' ? p[k].toFixed(2) : p[k]}</span></span>`);
              }
            });
          }
        }
